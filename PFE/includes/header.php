...
if (isset($_SESSION['user']['id'])) {
    ...
    // This is placeholder data. In a real application, you would query your
    // 'Notification' table based on the logged-in user's ID.
    // e.g., $stmt = $pdo->prepare("SELECT * FROM Notification WHERE idUtilisateur = ? AND is_read = 0 ORDER BY dateCreation DESC LIMIT 5");
    ...
    $notifications = [
        [
            ...
            'color' => 'text-success',
            'text' => '<strong>Session validée:</strong> Votre session "Bases du PHP" est confirmée pour demain.',
            'time' => 'Il y a 15 min'
        ],
        [
            'type' => 'new_message',
        ...
        [
            ...
            'color' => 'text-warning',
            'text' => '<strong>Paiement reçu:</strong> 25.00€ pour la session "Algorithmes I".',
            'time' => 'Hier'
        ],
    ];
