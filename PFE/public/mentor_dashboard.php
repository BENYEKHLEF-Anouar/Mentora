...
try {
    ...
    foreach($conversations_data as $convo) {
        if ($convo['estLue'] == 0 && $convo['idExpediteur'] != $mentor_user_id) {
    ...
    // Availability Data
    $stmt = $pdo->prepare("SELECT jourSemaine, TIME_FORMAT(heureDebut, '%H:%i') as heureDebut, TIME_FORMAT(heureFin, '%H:%i') as heureFin FROM Disponibilite WHERE idUtilisateur = ?");
    $stmt->execute([$mentor_user_id]);
...
// MODIFICATION: Require header, which includes DOCTYPE, <head>, and opening <body> tags.
require '../includes/header.php';
?>
<!-- Add page-specific <head> content here if needed, like scripts or meta tags -->
    ...
    <div class="dashboard-main-content">
        ...
        <div id="ressources" class="tab-content">
            ...
            <div class="form-card">
                ...
                <form action="actions/add_resource.php" method="POST" enctype="multipart/form-data"><input type="hidden" name="csrf_token" value="<?= $csrf_token ?>"><div class="form-group"><label for="titreRessource">Titre de la ressource</label><input type="text" id="titreRessource" name="titreRessource" placeholder="Ex: Exercices Corrigés" required></div><div class="form-group"><label for="fileUpload" class="file-upload-label"><i class="fas fa-cloud-upload-alt"></i> <span id="file-upload-text">Choisir un fichier</span></label><input type="file" id="fileUpload" name="fileUpload" hidden required></div><div class="form-actions"><button type="submit" class="btn-add-resource"><i class="fa-solid fa-plus"></i> Ajouter</button></div></form>
            </div>
            <div class="resource-list">
                <h4 class="tab-subtitle">Ressources existantes</h4>
                <?php if (empty($resources_data)): ?><p>Aucune ressource.</p><?php else: foreach ($resources_data as $res): ?>
                    <div class="resource-item" data-id="<?= $res['idRessource'] ?>"><i class="fas fa-file-alt resource-icon <?= $res['typeFichier'] ?>"></i><div class="resource-details"><p class="resource-title"><?= sanitize($res['titreRessource']) ?></p><p class="resource-info">Type: <?= sanitize($res['typeFichier']) ?></p></div><div class="resource-actions"><a href="actions/delete_resource.php?id=<?= $res['idRessource'] ?>&csrf_token=<?= $csrf_token ?>" class="action-btn delete" title="Supprimer" onclick="return confirm('Vraiment supprimer?');"><i class="fas fa-trash-alt"></i></a></div></div>
                <?php endforeach; endif; ?>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    // --- PRELOADER FIX ---
    // Hides the preloader once the page content is fully loaded.
    ...
    document.body.addEventListener('click', async (e) => {
        const sessionButton = e.target.closest('.btn-accept, .btn-decline');
        if(sessionButton) { handleSessionRequest(sessionButton); return; }
        const convoItem = e.target.closest('.conversation-item');
        if(convoItem) { handleConversationClick(convoItem); return; }
    });

    async function handleSessionRequest(button) {
        const action = button.classList.contains('btn-accept') ? 'accept' : 'decline';
        const card = button.closest('.session-request-card');
        ...
        try {
            ...
            button.innerHTML = `<i class="fas fa-${action === 'accept' ? 'check' : 'times'}"></i>`;
        }
    }
    ...
    async function handleConversationClick(convoItem) {
        document.querySelectorAll('.conversation-item').forEach(i => i.classList.remove('active'));
        convoItem.classList.add('active');
        
        activeChatUserId = convoItem.dataset.userId;
        chatHeaderName.textContent = convoItem.dataset.userName;
        messageArea.innerHTML = '<p class="empty-chat"><i class="fas fa-spinner fa-spin"></i> Loading...</p>';
        messageForm.style.display = 'flex';
        
        convoItem.querySelector('.unread-dot')?.remove();
        ...
        try {
            ...
            messageArea.innerHTML = `<p class="empty-chat">Error loading messages.</p>`;
            console.error(error);
        }
    }
    ...
    function renderMessages(messages) {
        ...
        if (messages.length === 0) {
            messageArea.innerHTML = '<p class="empty-chat">Start the conversation!</p>';
            return;
        }
        messages.forEach(msg => {
            const msgDiv = document.createElement('div');
            msgDiv.classList.add('chat-message');
            msgDiv.classList.toggle('message-outgoing', msg.idExpediteur == currentUserId);
            msgDiv.classList.toggle('message-incoming', msg.idExpediteur != currentUserId);
            const p = document.createElement('p');
            p.textContent = msg.contenuMessage;
    ...
    messageForm.addEventListener('submit', async (e) => {
        ...
        const tempMessage = textarea.value;
        textarea.value = '';
        
        const msgDiv = document.createElement('div');
        msgDiv.classList.add('chat-message', 'message-outgoing');
        const p = document.createElement('p');
        p.textContent = tempMessage;
        msgDiv.appendChild(p);
        ...
        try {
            ...
            formData.append('csrf_token', csrfToken);
            await fetch('actions/send_message.php', { method: 'POST', body: formData });
        } catch (error) {
