...
try {
    ...
    ");
    $stmt_stats->execute([$student_id, $student_id, $student_id]);
...
<main class="dashboard-container">
    <aside class="profile-sidebar">
        <div class="profile-card">
            <div class="card-image-container">
                <img src="<?= get_profile_image_path($student_info['photoUrl']) ?>" alt="Photo de profil de <?= sanitize($student_info['prenomUtilisateur']) ?>">
            </div>
            <div class="card-body">
                <h3 class="profile-name"><?= sanitize($student_info['prenomUtilisateur']) ?> <?= sanitize(substr($student_info['nomUtilisateur'], 0, 1)) ?>.</h3>
                <p class="profile-level"><?= sanitize($student_info['niveau']) ?></p>
                
                <div class="progression-stats">
                    <h4>Ma Progression</h4>
                    <ul>
                        <li><i class="fas fa-check-circle"></i><span><?= $stats['sessions_done_count'] ?></span> Sessions terminées</li>
                        <li><i class="fas fa-chalkboard-teacher"></i><span><?= $stats['mentors_contacted_count'] ?></span> Mentors contactés</li>
                        <li><i class="fas fa-book"></i><span><?= $stats['subjects_studied_count'] ?></span> Matières étudiées</li>
                    </ul>
                </div>
                ...
                <div class="badge-showcase">
                    ...
                    <div class="badges-grid">
                        <?php if (empty($badges)): ?><p class="no-badges">Aucun badge pour le moment.</p><?php else: foreach ($badges as $badge): ?>
            ...
            <div class="card-footer">
                <a href="edit_profile2.php" class="btn-secondary"><i class="fas fa-pencil-alt"></i> Modifier le profil</a>
                <a href="mentors.php" class="btn-primary"><i class="fas fa-search"></i> Trouver un Mentor</a>
            </div>
        </div>
    </aside>
    ...
    <ul>
        <li><a href="#tableau-de-bord" class="dashboard-tab active" data-tab="tableau-de-bord"><i class="fas fa-tachometer-alt"></i> Tableau de Bord</a></li>
        <li><a href="#mes-sessions" class="dashboard-tab" data-tab="mes-sessions"><i class="fas fa-tasks"></i> Mes Sessions</a></li>
        <li><a href="#messagerie" class="dashboard-tab" data-tab="messagerie"><i class="fas fa-envelope"></i> Messagerie</a></li>
        <!-- <li><a href="#mes-mentors" class="dashboard-tab" data-tab="mes-mentors"><i class="fas fa-users"></i> Mes Mentors</a></li> -->
        <!-- NEW TAB LINK -->
        <li><a href="#disponibilites" class="dashboard-tab" data-tab="disponibilites"><i class="fas fa-calendar-alt"></i> Disponibilités</a></li>
    </ul>
     <?php if ($evaluations_to_give_count > 0): ?>
        <a href="#mes-sessions" class="evaluations-link tab-link" data-tab="mes-sessions" title="Vous avez des sessions à évaluer">
            <i class="fas fa-star"></i> Évaluations à donner
            <span class="notification-badge"><?= $evaluations_to_give_count ?></span>
        </a>
    <?php endif; ?>
</nav>
        
        <div id="feedback-container-global" style="display: none; margin: 1.5rem 2.5rem 0 2.5rem;"></div>

        <!-- Tableau de Bord Tab -->
        <div id="tableau-de-bord" class="tab-content active">
            <h2 class="tab-title">Bienvenue, <?= sanitize($student_info['prenomUtilisateur']) ?> !</h2>
            <div class="dashboard-grid">
                <div class="info-card">
                    ...
                    <?php else: ?>
                        ...
                        <a href="mentors.php" class="btn-primary-small">Trouver un Mentor</a>
                    <?php endif; ?>
                </div>
                <div class="info-card">
        ...
        <div id="mes-sessions" class="tab-content">
            <h2 class="tab-title">Historique de vos sessions</h2>
        ...
        <div id="disponibilites" class="tab-content">
            <h2 class="tab-title">Gérez vos Disponibilités</h2>
            <div class="availability-card">
                <div class="availability-header">
                    <h4><i class="far fa-calendar-check"></i> Vos disponibilités hebdomadaires</h4>
                    <p>Indiquez les créneaux où vous êtes généralement disponible pour des sessions.</p>
                </div>
                <form id="availability-form">
                    <input type="hidden" name="csrf_token" value="<?= $csrf_token ?>">
                    <div id="availability-feedback" style="display: none; margin: 1.5rem 1.5rem 0;"></div>
                    <div class="availability-grid">
                        <div class="grid-header">Heure</div>
                        <?php foreach ($days_of_week as $day): ?>
                            <div class="grid-header"><?= sanitize($day) ?></div>
                        <?php endforeach; ?>
                        ...
                        <?php foreach ($time_slots as $slot): ?>
                            ...
                            <?php foreach ($days_of_week as $day): ?>
                                ...
                                ?>
                                <div class="time-slot">
                                    <input type="checkbox" name="slots[<?= $day_db_format ?>][]" value="<?= $slot ?>" <?= $is_available ? 'checked' : '' ?> id="slot-student-<?= $day_db_format ?>-<?= str_replace(':', '', $slot) ?>">
                                    <label for="slot-student-<?= $day_db_format ?>-<?= str_replace(':', '', $slot) ?>"></label>
                                </div>
                            <?php endforeach; ?>
                        <?php endforeach; ?>
                    </div>
                    <div class="availability-actions">
                        <button type="submit" class="btn-save-availability">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    activateTab(window.location.hash.substring(1) || 'tableau-de-bord');

    // --- Event Delegation ---
    document.body.addEventListener('click', e => {
        if (e.target.closest('.btn-cancel')) handleCancelSession(e.target.closest('.btn-cancel'));
        if (e.target.closest('.btn-evaluate')) openEvaluationModal(e.target.closest('.btn-evaluate'));
        if (e.target.closest('.modal-overlay:not(.modal-content)') || e.target.closest('.modal-close-btn')) document.getElementById('evaluation-modal').style.display = 'none';
        if (e.target.closest('.conversation-item')) handleConversationClick(e.target.closest('.conversation-item'));
        if (e.target.closest('.contact-from-grid')) handleContactFromGrid(e.target.closest('.contact-from-grid'));
    });
