<?php
// ... (Votre code existant de finish_register.php de la ligne 1 à 157 reste le même)
// La fonction get_field_class est déjà ici et correcte.
        ...
        if (!empty($availabilities)) {
            foreach ($availabilities as $day => $times) {
                if (!empty($times['enabled'])) {
                    ...
                    // Si start ou end manquant -> erreur
                    if (empty($times['start']) || empty($times['end'])) {
                        $errors['availabilities'] = "Veuillez renseigner les horaires pour " . ucfirst($day) . ".";
                    } else {
                        // On crée des objets DateTime en format 24h
                        $start = DateTime::createFromFormat('H:i', $times['start']);
                        $end = DateTime::createFromFormat('H:i', $times['end']);
                        if (!$start || !$end) {
                            $errors['availabilities'] = "Format de temps invalide pour " . ucfirst($day) . ". Utilisez HH:MM en 24h.";
                        } elseif ($start >= $end) {
...
if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
    ...
    $photoFileName = 'default_avatar.png'; // Fallback if upload fails
}
?>
...
<img id="photo-preview" 
     ...
     style="display:<?= $displayStyle ?>; max-width:100px; margin-top:10px; border-radius: 8px;">
                        <?php if (!empty($errors['photo'])): ?><p class="error-message"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['photo']) ?></p><?php endif; ?>
                    </div>
                    <div class="input-group">
                        ...
                        <?php if (!empty($errors['ville'])): ?><p class="error-message"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['ville']) ?></p><?php endif; ?>
                    </div>
                </div>
                ...
                <div class="input-group">
                    <?php if ($user['role'] === 'etudiant'): ?>
                        ...
                        <?php if (!empty($errors['niveau'])): ?><p class="error-message"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['niveau']) ?></p><?php endif; ?>
                        ...
                        <?php if (!empty($errors['sujets'])): ?><p class="error-message"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['sujets']) ?></p><?php endif; ?>
                    ...
                    <?php elseif ($user['role'] === 'mentor'): ?>
                        ...
                        <?php if (!empty($errors['competences'])): ?><p class="error-message"><i class="fa-solid fa-circle-exclamation"></i> <?= htmlspecialchars($errors['competences']) ?></p><?php endif; ?>
                    <?php endif; ?>
                </div>
                ...
                <div class="input-group">
                    ...
                    <div class="availability-group <?= get_field_class('availabilities') ?>">
                        ...
                        foreach ($days as $day):
                            $isEnabled = isset($availabilities[$day]['enabled']);
                            $startTime = $availabilities[$day]['start'] ?? '';
                            $endTime = $availabilities[$day]['end'] ?? '';
                        ?>
                        <div class="availability-row">
                            <label class="day-label">
                                <input type="checkbox" name="availabilities[<?= $day ?>][enabled]" value="1" <?= $isEnabled ? 'checked' : '' ?>>
                                <span><?= ucfirst($day) ?></span>
                            </label>
                            <div class="time-input-wrapper">
                                <!-- 24-hour format time input, with pattern et step -->
                                <input 
                                    type="time" 
                                    name="availabilities[<?= $day ?>][start]" 
