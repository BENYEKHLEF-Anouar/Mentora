...
function formatDuration($minutes) {
    ...
    if ($rem_minutes > 0) {
        return $hours . 'h' . str_pad($rem_minutes, 2, '0', STR_PAD_LEFT);
    }
    return $hours . 'h';
}

// --- FIXED Helper function to generate CSS class from subject ---
function getSessionStyleClass($subject) {
    if (!$subject) return 'session-card--default';
    // Basic transliteration for common French accents and spaces
    $subject = str_replace(
        ['é', 'è', 'ê', 'à', 'ç', 'ô', 'î', 'û', ' '],
        ['e', 'e', 'e', 'a', 'c', 'o', 'i', 'u', '-'],
        $subject
...
// --- FETCH DATA FOR MENTORS WITH AVAILABILITY (no change) ---
...
$stmtMentors = $pdo->prepare("
    SELECT
        ...
        COUNT(p.notation) AS review_count,
        CASE WHEN EXISTS (
...
// --- REWRITTEN: FETCH DATA FOR AVAILABLE SESSIONS ---
$stmtSessions = $pdo->query("
    SELECT
        ...
        s.duree_minutes,
        s.niveau,
        -- Get the hosting mentor's information
    ...
    JOIN Utilisateur u ON m.idUtilisateur = u.idUtilisateur
    WHERE s.statutSession = 'en_attente' AND s.idMentorAnimateur IS NOT NULL
...
<!-- MENTORS SECTION - MODIFIED FOR AVAILABILITY -->
    ...
    <div class="container">
        ...
        <div class="profile-grid">
            <?php if (empty($mentors)): ?>
                <p>Aucun mentor n'est disponible pour le moment.</p>
            <?php else: ?>
                <?php foreach ($mentors as $mentor): ?>
                    <div class="profile-card" data-aos="fade-up" data-aos-delay="200">
                        <div class="card-image-container">
                            <img src="<?= htmlspecialchars($mentor['photoUrl']) ?>" alt="<?= htmlspecialchars($mentor['prenomUtilisateur']) ?>">
                        </div>
                        <div class="card-body">
                        ...
                        <div class="card-footer">
                            <!-- MODIFICATION: Dynamic Availability Status -->
                            <?php if ($mentor['is_available']): ?>
                                <span><span class="status-dot available"></span> Disponible</span>
                            <?php else: ?>
                                <span><span class="status-dot busy"></span> Occupé</span>
                            <?php endif; ?>
            ...
            <div class="container">
                ...
                <div class="sessions-list">
                    <?php if (empty($sessions)): ?>
                        <p>Aucune session de groupe n'est actuellement programmée.</p>
                    <?php else: ?>
                            ...
                            <div class="session-card <?= getSessionStyleClass($session['sujet']) ?>" data-aos="fade-up">
                                ...
                                <div class="session-body">
                                    <div class="session-host">
                                        <img src="<?= htmlspecialchars($session['mentor_photo']) ?>" alt="Photo de <?= htmlspecialchars($session['mentor_prenom']) ?>" class="host-avatar">
                                        <div class="host-info">
